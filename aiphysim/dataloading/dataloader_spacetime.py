...
class RB2DataLoader(Dataset):
    ...
    def __init__(
        self,
        data_dir="./",
        data_filename="./data/rb2d_ra1e6_s42.npz",
        nx=128,
        nz=128,
        nt=16,
        ...
        self.data = np.stack(
            [npdata["p"], npdata["b"], npdata["u"], npdata["w"]], axis=0
        )
        ...
        if (nx > nx_data) or (nz > nz_data) or (nt > nt_data):
            raise ValueError(
                "Resolution in each spatial temporal dimension x ({}), z({}), t({})"
        ...
        self.scale_hres = np.array(
            [self.nt_hres, self.nz_hres, self.nx_hres], dtype=np.int32
        )
        self.scale_lres = np.array(
            [self.nt_lres, self.nz_lres, self.nx_lres], dtype=np.int32
        )
    ...
    def filter(self, signal):
        ...
        if self.lres_filter == "none" or (not self.lres_filter):
            ...
            sigma = [
                0,
                int(self.downsamp_t / 2),
                int(self.downsamp_xz / 2),
                int(self.downsamp_xz / 2),
            ]
    ...
    def __getitem__(self, idx):
        ...
        interp = RegularGridInterpolator(
            ...
            values=space_time_crop_hres_fil.transpose(1, 2, 3, 0),
    ...
    def normalize_grid(self, grid):
        ...
        mean_bc = self.channel_mean[
            (...,) + (None,) * (g_dim - 1)
        ]  # unsqueeze from the back
        std_bc = self.channel_std[
            (...,) + (None,) * (g_dim - 1)
        ]  # unsqueeze from the back
...
if __name__ == "__main__":
    ...
    for batch_idx, (
        ...
        point_coords,
        point_values,
    ) in enumerate(data_batches):
        print(
            "Reading batch #{}:\t with lowres inputs of size {}, sample coord of size {}, sampe val of size {}".format(
                batch_idx + 1,
                list(lowres_input_batch.shape),
                list(point_coords.shape),
                list(point_values.shape),
            )
        )
        if batch_idx > 16:
            break
