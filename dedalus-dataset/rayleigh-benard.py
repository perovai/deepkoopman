...
def get_args():
    parser = argparse.ArgumentParser(description='Data generation script for 2D Rayleigh-Benard convection using Dedalus')
    ...
    parser.add_argument('--rayleigh', default=15e4, type=float,
                        help='Simulation Rayleigh number. (default: 15e4)')
    parser.add_argument('--prandtl', default=5, type=float,
...
if __name__ == '__main__':
    ...
    snapshots.add_task("0.5 * (u ** 2 + w ** 2)", layout='g', name='KE')  # Kinetic energy
    snapshots.add_task("sqrt(u ** 2 + w ** 2)", layout='g', name='|uvec|')  # Norm of the velocity
